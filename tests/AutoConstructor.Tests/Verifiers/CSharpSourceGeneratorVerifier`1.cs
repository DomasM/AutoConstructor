    ...
    where TSourceGenerator : IIncrementalGenerator, new()
{
    public static Task RunAsync(
        ...
        bool expectSerializerConstructor = false)
    {
        return RunAsync(code, new[] { (generated, generatedName) }, nullable, diagnostics, configFileContent, additionalProjectsSource, runWithNullChecks, expectSerializerConstructor);
    }
        ...
        bool expectSerializerConstructor = false)
    {
            ...
            TestState =
                {
                    ...
                    GeneratedSources =
                    {
                        ...
                        (typeof(AutoConstructorGenerator), $"{Source.SerializationAttributeFullName}.cs", SourceText.From(Source.SerializerAttributeText, Encoding.UTF8)),
        ...
        var beforeCtorBoilerPlateLines = new List<string> { @$"[global::System.CodeDom.Compiler.GeneratedCodeAttribute(""{nameof(AutoConstructor)}"", ""{AutoConstructorGenerator.GeneratorVersion}"")]" };
        if (expectSerializerConstructor)
        {
            beforeCtorBoilerPlateLines.Add("#pragma warning disable CS8618 // Non-nullable field must contain a non-null value when exiting constructor. Consider declaring as nullable.");
            beforeCtorBoilerPlateLines.Add(@"[global::System.ObsoleteAttribute(""For serialization only"",true)]");
        }
        foreach ((string? generated, string generatedName) in generatedSources)
        {
            if (generated is string { Length: > 0 })
            {
                string beforeCtorWhitespace = BeforeConstructorRegex().Match(generated).Groups[1].Value;
                string beforeCtorBoilerPlate = string.Join(beforeCtorWhitespace, beforeCtorBoilerPlateLines);
...
{generated}";
                generatedWithHeader = BeforeConstructorRegex()
                    .Replace(generatedWithHeader, $"$1{beforeCtorBoilerPlate}$1$2 $3(")
